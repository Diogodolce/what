...
const dirStop = './bot';
if (!fs.existsSync(dirStop)){
  fs.mkdirSync(dirStop)
}
// TEMPO MS (MILESEGUNDO)
...
client.on('message', async msg => {
  ...
  const dirStopFrom = './bot/' + msg.from.replace(/\D/g,'');
  if (!fs.existsSync(dirFrom)){
    await createSession(msg);
  }
  async function readWriteFileJsonStop(botStatus) {
    let dataFile = [];
    fs.writeFileSync("./bot/" + msg.from.replace(/\D/g,'') + "/bot.json", JSON.stringify(dataFile));
    var data = fs.readFileSync("./bot/" + msg.from.replace(/\D/g,'') + "/bot.json");
    var myObject = JSON.parse(data);
    let newData = {
      status: botStatus,
    };
    await myObject.push(newData);
    fs.writeFileSync("./bot/" + msg.from.replace(/\D/g,'') + "/bot.json", JSON.stringify(myObject));
  }
  if (!fs.existsSync(dirStopFrom)){
    fs.mkdirSync(dirStopFrom);
    await readWriteFileJsonStop("on");
  }
  
  try {
    const status = fs.readFileSync("./bot/" + msg.from.replace(/\D/g,'') + "/bot.json","utf8").split(':')[1].replace(/\W/g, '');
    
    if (msg.body !== 'reiniciar' && msg.body !== 'desligar' && status === 'on'){
    ...
    if (msg.body === 'reiniciar' && status === 'on'){
      deleteDirectory(dirFrom)
      await client.sendMessage(msg.from, '*BLACKSIDER API:*\nAtendimento automático reiniciado.')
    }

    if (msg.body === 'desligar'){
      await readWriteFileJsonStop('off');
      delay(tempoBot).then(async function() {
        await readWriteFileJsonStop('on');
        msg.reply("*BLACKSIDER API:*\nAtendimento religado.");
      });
